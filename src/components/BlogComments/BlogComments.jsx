...
function BlogComments({ Id }) {
    ...
    }, [Id, setBlog, setComments, getSingleBlog])
        ...
        <div style={{ padding: 15 }}>
            ...
            {comments.map(comment => (
                <div key={comment._id} style={{ marginBottom: '1rem' }}>
                    <Comment comment={comment} Id={blog._id} userId={userId} />
                    {comment.replies && comment.replies.length > 0 && (
                        <div className="replies" style={{ marginLeft: '1rem', display: 'flex', flexDirection: 'column-reverse' }}>
                            {comment.replies.map(reply => (
                                <div key={reply._id} className="reply">
                                    <ReplyCard comment={reply} Id={blog._id} commentId={comment._id} userId={userId} />
                                </div>
                            ))}
                        </div>
                    )}
            ...
            <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
                ...
                <Button onClick={() => addComment(Id, text, userId)}>Post</Button>
            </div>
        </div >
    )
}
                ...
                <span>
                    {userId === comment.user._id ? (
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                            ...
                            <Button onClick={() => deleteComment(Id, comment._id)}>Delete</Button>
                        </div>
            ...
            <div>
                ...
                {showReply && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        ...
                        <Button style={{ marginTop: 10, alignSelf: 'flex-end' }} onClick={() => replyToComment(Id, comment._id, replyText, userId)}>Post Reply</Button>
                    </div>
                )}
                ...
                {showEdit && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        ...
                        />
                        <Button style={{ marginTop: 10, alignSelf: 'flex-end' }} onClick={() => editComment(Id, comment._id, editText)}>Edit Comment</Button>
                    </div>
                )}
            </div>
        ...
        <div className="comment" style={{ width: '100%', padding: 15, display: 'flex', flexDirection: 'column', gap: 15, justifyContent: 'center' }}>
            ...
            <div>
                ...
                {showReply && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <Input placeholder='Add a reply' value={replyText} onChange={(e) => setReplyText(e.target.value)} />
                        <Button style={{ marginTop: 10, alignSelf: 'flex-end' }} onClick={() => replyToReply(Id, commentId, comment._id, replyText, userId)}>Post Reply</Button>
                    </div>
                )}
